...
export const EditableDiff = () => {
  ...
  const createDiffState = (
    ...
    let nextEditorState = EditorState.createWithContent(
      ContentState.createFromText(article),
      decorator
    );

    let nextContent = nextEditorState.getCurrentContent();

    fragmentsWithIndex.forEach((fragment: Diff) => {
      if (fragment.value !== "\n" && (fragment.added || fragment.removed)) {
        const blockKey = nextContent
          .getBlocksAsArray()
          [fragment.blockIndex].getKey();
        ...
        const contentStateWithEntity = nextContent.createEntity(
          type,
          "MUTABLE",
          {
            text: fragment.value
    ...
    setResultState(nextEditorState);
  };
  ...
  return (
    <Box sx={{ display: "grid", gridGap: "10px" }}>
      ...
      >
        <Button
          variant="contained"
          onMouseDown={() => createDiffState(newState, currentState)}
        >
          差分比較
        </Button>
